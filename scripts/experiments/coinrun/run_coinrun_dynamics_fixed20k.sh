...
run_one() {
  ...
  .venv/bin/python scripts/train_dynamics.py \
    ...
    write_video_every=0 \
    run_name="coinrun-dynamics-${name}-fixed20k" \
...
# Largest first: it gives the earliest answer to whether the 474-step run was
# simply undertrained. All sizes receive the same optimizer-step curriculum.
run_one medium 4 256 4 32
run_one small 2 128 2 16
run_one tiny 2 64 1 8
